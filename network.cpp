...
void Network::loadDB(string filename){
    ...
    Person *prev_temp;
    int line_num = 1;
    ptr = new Person(filename, line_num);
    count++;
    head = ptr;
    ptr->prev = NULL;
    ...
    while (!ifile.eof()){
        cout << "starting new db..." << endl;
        //ptr->set_person(filename, line_num);

        if (ptr != head) ptr->prev = prev_temp;

        next_temp = new Person(filename, line_num);
        for (int i = 0; i <= 5; i++){
            getline(ifile, trash);
            line_num++;
            cout << trash << endl;
